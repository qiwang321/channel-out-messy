...
void finetune_rnorm_competeAbs() {
	////assignOpt();
	printf("starting finetune_rnorm_competeAbs()! keepProb = %f, perturbScale = %f\n", keepProb, perturbScale);
	...
	printf("inputKeepProb--%f\n", opt1.keepProbInput);
	printf("inputScale--%f\n", opt1.inputScale);
	fprintf(pFile, "starting finetune_rnorm_competeAbs()!\nkeepProb = %f\nperturbScale = %f\nwiderScale = %f\ninputKeepProb = %f\ninputScale = %f\n", keepProb, perturbScale, widerScale, opt1.keepProbInput, opt1.inputScale);
...
//	NVMatrix switchMask0, switchMask1, switchMask2, switchMask3, switchMask4;
//	NVMatrix dropMask0;
...
//			cout << "0->1\n";
			...
			dropMask0.randomizeBinary(opt1.keepProbInput); // input part
			act0.eltwiseMult(dropMask0);
			act0.scale(opt1.inputScale);
			act0.scale(1.0 / opt1.keepProbInput); // very, very important!
			activateConv(act0, act1, weight1, bias1, opt1);
			act1Pool.transpose(false);
			convLocalPool(act1, act1Pool, opt1.numFilters, opt1.poolSize, opt1.poolStartX, opt1.poolStride, opt1.poolOutX, MaxAbsPooler());
			convCompeteAbs(act1Pool, act1PoolMax, maxMask1, opt1.numFilters, opt1.maxOutPoolSize, opt1.maxOutPoolStride, opt1.poolOutX, batchSize);
...
//			cout << "1->2\n";
			...
			//switchMask1.resize(act1PoolMax);
			//switchMask1.randomizeTernary();
			//act1PoolMax.eltwiseMult(switchMask1);
			//widerMask1.resize(act1PoolMax);
			//widerMask1.randomizeBinaryWider(widerScale);
			//act1PoolMax.eltwiseMult(widerMask1);


			activateConv(act1PoolMax, act2, weight2, bias2, opt2);
			act2Pool.transpose(false);
			convLocalPool(act2, act2Pool, opt2.numFilters, opt2.poolSize, opt2.poolStartX, opt2.poolStride, opt2.poolOutX, MaxAbsPooler());
			convCompeteAbs(act2Pool, act2PoolMax, maxMask2, opt2.numFilters, opt2.maxOutPoolSize, opt2.maxOutPoolStride, opt2.poolOutX, batchSize);
...
//			cout << "2->3\n";
			...
			//switchMask2.resize(act2PoolMax);
			//switchMask2.randomizeTernary();
			//act2PoolMax.eltwiseMult(switchMask2);
			//widerMask2.resize(act2PoolMax);
			//widerMask2.randomizeBinaryWider(widerScale);
			//act2PoolMax.eltwiseMult(widerMask2);
			...
			convLocalPool(act3, act3Pool, opt3.numFilters, opt3.poolSize, opt3.poolStartX, opt3.poolStride, opt3.poolOutX, MaxAbsPooler());
			convCompeteAbs(act3Pool, act3PoolMax, maxMask3, opt3.numFilters, opt3.maxOutPoolSize, opt3.maxOutPoolStride, opt3.poolOutX, batchSize);
...
//			cout << "3->4\n";
			...
			//switchMask3.resize(act3PoolMax);
			//switchMask3.randomizeTernary();
			//act3PoolMax.eltwiseMult(switchMask3);
			//widerMask3.resize(act3PoolMax);
			//widerMask3.randomizeBinaryWider(widerScale);
			//act3PoolMax.eltwiseMult(widerMask3);

			act3PoolMax.transpose(true);
			activate(act3PoolMax, act4, weight4, bias4, 0, 1);
			act3PoolMax.transpose(false);
			act4.transpose(false);
			convCompeteAbs(act4, act4Max, maxMask4, opt4.numFilters/opt4.outX/opt4.outX, opt4.maxOutPoolSize, opt4.maxOutPoolStride, opt4.outX, batchSize);
...
//			cout << "4->top\n";
			...
			//switchMask4.resize(act4Max);
			//switchMask4.randomizeTernary();
			//act4Max.eltwiseMult(switchMask4);
			//widerMask4.resize(act4Max);
			//widerMask4.randomizeBinaryWider(widerScale);
			//act4Max.eltwiseMult(widerMask4);
...
//			cout << "4->3\n";
			...
			//act4MaxGrad.eltwiseMult(switchMask4);
			//act4MaxGrad.eltwiseMult(widerMask4);
...
//			cout << "3->2\n";
			...
			//act3PoolMaxGrad.eltwiseMult(switchMask3);
			//act3PoolMaxGrad.eltwiseMult(widerMask3);
...
//			cout << "2->1\n";
			...
			//act2PoolMaxGrad.eltwiseMult(switchMask2);
			//act2PoolMaxGrad.eltwiseMult(widerMask2);
...
//			cout << "1->0\n";
			...
			//act1PoolMaxGrad.eltwiseMult(switchMask1);
			//act1PoolMaxGrad.eltwiseMult(widerMask1);
	...
	printf("finetune_rnorm_competeAbs() complete!\n");
	fprintf(pFile, "finetune_rnorm_competeAbs() complete!\n");
...
void multiViewTest_competeAbs() {
	////assignOpt();
	printf("starting multiViewTest_competeAbs()!\n");
	fprintf(pFile, "starting multiViewTest_competeAbs()!\n");
	...
	// data and parameters storage
	NVMatrix act0, act1, act1Pool, act1PoolMax;
	NVMatrix act2, act2Pool, act2PoolMax;
	NVMatrix act3, act3Pool, act3PoolMax;
	NVMatrix act4, act4Max;
	NVMatrix actTop;
	...
	for (int batch = 0; batch < CPUTest.size(); batch++) {
		...
		for (int r = 0; r < opt1.numViews; r++) {
			...
			//cout << "0->1\n";
			//original
			GPUTest[batch*opt1.numViews+r]->copy(act0);
			act0.scale(opt1.inputScale);
			activateConv(act0, act1, weight1, bias1, opt1);
			act1Pool.transpose(false);
...
//			cout << "4->top\n";
	...
	printf("\ntest set precision: %f\n; objective = %f; time elapsed = %f seconds\n", cost, cost1,
			(float)(clock() - startClock)/CLOCKS_PER_SEC);
	printf("multiViewTest_competeAbs() complete!\n");

	fprintf(pFile, "\ntest set precision: %f\n; objective = %f; time elapsed = %f seconds\n", cost, cost1,
			(float)(clock() - startClock)/CLOCKS_PER_SEC);
	fprintf(pFile, "multiViewTest_competeAbs() complete!\n");
