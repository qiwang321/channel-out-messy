...
void assignOpt() {
	strcpy(opt1.testName, "competeOut");
	...
	opt1.poolOutX = (opt1.outX - opt1.poolStartX - 1) / opt1.poolStride + 1;
	...
	opt1.maxOutPoolSize = 2;
	opt1.maxOutPoolStride = 2;
	opt1.numGroups = (opt1.numFilters - 1) / opt1.maxOutPoolStride + 1;
	opt1.dataPath.assign("/scratch0/qwang37/cifar-10-batches-bin");
	opt1.weightPath.assign("/scratch0/qwang37/weights_cifar10_10_25_maxtop_24");
	opt1.exp.assign("cifar10");
	// weight regularization
	opt1.maxNorm = 0.9;
	strcpy(opt1.lrDecayType, "exponential");
	opt1.lrStartScale = 1.5;
	opt1.lrDecayFactor = 0.95;
	opt1.lrMinRate = 0.01;
	strcpy(opt1.momIncType, "linear");
	opt1.momStartScale = 1.0;
	opt1.momIncFactor = 0.000;
	opt1.momMaxRate = 1.0;

	opt1.keepStartRate = 0.6;
	opt1.keepIncRate = 0.000;
	opt1.keepEndRate = 0.6;

	opt1.keepInputStartRate = 1.0;
	opt1.keepInputEndRate = 1.0;
	opt1.keepInputIncRate = 0.000;
	...
	opt2.poolOutX = (opt2.outX - opt2.poolStartX - 1) / opt2.poolStride + 1;
	...
	//maxout
	opt2.maxOutPoolSize = 2;
	opt2.maxOutPoolStride = 2;
	...
	opt3.poolOutX = (opt3.outX - opt3.poolStartX - 1) / opt3.poolStride + 1;
	...
	opt4.numVis = opt3.poolOutX * opt3.poolOutX * opt4.numChannels;
	opt4.numFilters = 1024;
	opt4.paddingStart = 0;
	opt4.initstv = 0.005;
	opt4.mom = opt1.mom;
	opt4.lrW = 0.1;
	...
	opt4.maxOutPoolSize = 4;
	opt4.maxOutPoolStride = 4;
	...
	optTop.numFilters = 40; // classification nodes
	...
	optTop.maxOutPoolSize = optTop.numFilters / opt1.labelSize;
	optTop.maxOutPoolStride = optTop.maxOutPoolSize;
