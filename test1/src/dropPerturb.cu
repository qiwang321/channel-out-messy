...
void finetune_rnorm_dropPerturb() {
	////assignOpt();
	printf("starting finetune_rnorm_dropPerturb()! perturbScale = %f\n", perturbScale);
	fprintf(pFile, "starting finetune_rnorm_dropPerturb()! perturbScale = %f\n", perturbScale);
	...
	for (int epoch = 0; epoch < opt1.numEpochs; epoch++) {
		...
		for (int batch = 0; batch < GPUTrain.size(); batch++) {
			...
			act1.addUniformNoise(perturbScale);
			...
			act2.addUniformNoise(perturbScale);
			...
			act3.addUniformNoise(perturbScale);
			...
			act4.addUniformNoise(perturbScale);
	...
	printf("finetuning_rnorm_dropPerturb() complete! perturbScale = %f\n", perturbScale);
	fprintf(pFile, "finetuning_rnorm_dropPerturb() complete! perturbScale = %f\n", perturbScale);
} // int finetune_rnorm()


void multiViewTest_dropPerturb() {
	////assignOpt();
	printf("starting multiViewTest_dropPerturb()!\n");
	fprintf(pFile, "starting multiViewTest_dropPerturb()!\n");
	...
	printf("\ntest set precision: %f\n; objective = %f; time elapsed = %f seconds\n", cost, cost1,
			(float)(clock() - startClock)/CLOCKS_PER_SEC);
	printf("multiViewTest_dropPerturb() complete!\n");

	fprintf(pFile, "\ntest set precision: %f\n; objective = %f; time elapsed = %f seconds\n", cost, cost1,
			(float)(clock() - startClock)/CLOCKS_PER_SEC);
	fprintf(pFile, "multiViewTest_dropPerturb() complete!\n");
} // void multiViewTest()
